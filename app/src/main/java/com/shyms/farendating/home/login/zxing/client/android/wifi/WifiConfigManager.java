...
package com.shyms.farendating.home.login.zxing.client.android.wifi;

import android.net.wifi.WifiConfiguration;
import android.net.wifi.WifiManager;
import android.os.AsyncTask;

import com.shyms.farendating.home.login.zxing.client.result.WifiParsedResult;
...
public final class WifiConfigManager extends AsyncTask<WifiParsedResult,Object,Object> {
  ...
  protected Object doInBackground(WifiParsedResult... args) {
    ...
    if (!wifiManager.isWifiEnabled()) {
      LogcatUtil.i(TAG, "Enabling wi-fi...");
      if (wifiManager.setWifiEnabled(true)) {
        LogcatUtil.i(TAG, "Wi-fi enabled");
      } else {
        LogcatUtil.w(TAG, "Wi-fi could not be enabled!");
        return null;
      }
      ...
      while (!wifiManager.isWifiEnabled()) {
        if (count >= 10) {
          LogcatUtil.i(TAG, "Took too long to enable wi-fi, quitting");
          return null;
        }
        LogcatUtil.i(TAG, "Still waiting for wi-fi to enable...");
        try {
          Thread.sleep(1000L);
    ...
    try {
      ...
      LogcatUtil.w(TAG, "Bad network type; see NetworkType values: " + networkTypeString);
      return null;
    }
  ...
  private static void updateNetwork(WifiManager wifiManager, WifiConfiguration config) {
    ...
    if (foundNetworkID != null) {
      LogcatUtil.i(TAG, "Removing old configuration for network " + config.SSID);
    ...
    if (networkId >= 0) {
      ...
      if (wifiManager.enableNetwork(networkId, true)) {
        LogcatUtil.i(TAG, "Associating to network " + config.SSID);
        wifiManager.saveConfiguration();
      } else {
        LogcatUtil.w(TAG, "Failed to enable network " + config.SSID);
      }
    } else {
      LogcatUtil.w(TAG, "Unable to add network " + config.SSID);
    }
  }
